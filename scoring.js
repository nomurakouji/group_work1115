...
$(document).ready(function () {
    ...
    function  get_sum (){
        let box = score_indicate(); //boxの中にscore_indicateを挿入。戻り値としてboxに中身が入る（配列を返す）
        let sum_number = box.length; //長さが入る
        let sum = 0; //空の箱を用意するには宣言する
        for (let i = 0 ; i < sum_number; i++) {
            sum += box[i];
        }
        return sum;
    };
    ...
    function set_sum() {
        $("#sum_indicate").text(get_sum());
    };
    
    function get_average (){
        let number = score_indicate().length; //割るべき個数を出す
        let sum = get_sum(); //点数合計が戻り値
        let average = sum / number;
        return average;
    };

    function set_average() {
        $("#average_indicate").text(get_average());
    };
    ...
    function get_achievement() {
      ...
      // 平均点数をHTML上のid="average_indicate"から取得して代入します。
        let averageIndicate = get_average();
    ...
    function get_pass_or_failure() {
        let box = score_indicate();
      // 変数「number」に入力した教科の数を代入します。
      let number = box.length;
